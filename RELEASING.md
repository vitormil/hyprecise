...
| Bump | When |
|---|---|
| `patch` | A bug fix. Nothing about `setup()` or `keys` changes. |
| `minor` | A new option or behaviour, with nothing that already worked breaking. |
| `major` | A breaking change to `setup()` or `keys`. |

Those two are the supported surface, fixed at 1.0.0. Everything else the
